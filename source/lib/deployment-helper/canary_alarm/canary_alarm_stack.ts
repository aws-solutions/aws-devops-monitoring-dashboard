...
import {Canary, CfnCanary, Code, Runtime, Schedule, Test} from '@aws-cdk/aws-synthetics';
import {
  BlockPublicAccess,
  Bucket,
  BucketAccessControl,
...
import {AnyPrincipal, CfnRole, Effect, PolicyStatement} from '@aws-cdk/aws-iam';
...
export class CanaryStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: CanaryProps) {
    ...
    const paramCreateBucket = new CfnParameter(this, 'CreateBucket', {
      description: "Canaries store artifacts in an S3 bucket. " +
    ...
    const paramBucketName = new CfnParameter(this, 'BucketName', {
      description: "Name of the bucket (new or existing) for logging Canary artifacts. " +
    ...
    bktArtifact.addToResourcePolicy(
      new PolicyStatement({
        sid: 'HttpsOnly',
        resources: [
          bktArtifact.bucketArn,
          bktArtifact.arnForObjects('*')
        ],
        actions: [
          "s3:*"
        ],
    ...
    //
    let artifactBucket: IBucket = Bucket.fromBucketAttributes(this, "ArtifactiBucket", {
        bucketName: paramBucketName.valueAsString
    });
    ...
    let refCanary = canary.node.defaultChild as CfnCanary
